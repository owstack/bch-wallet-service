...
/**
 * Creates an instance of the Bcccore Wallet Service.
 * @constructor
 */
...
/**
 * Gets the current version of BCCWS
 */
WalletService.getServiceVersion = function() {
  if (!serviceVersion)
    serviceVersion = 'bccws-' + require('../package').version;
...
WalletService.getInstance = function(opts) {
  ...
  if (version && version.agent == 'bccwc') {
    if (version.major == 0 || (version.major == 1 && version.minor < 2)) {
      throw new ClientError(Errors.codes.UPGRADE_NEEDED, 'BCCWC clients < 1.2 are no longer supported.');
    }
  }
 ...
 * @param {string} opts.network[=Constants.LIVENET] - The Bitcoin Cash network for this wallet.
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new Bcccore.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype._verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new Bcccore.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
WalletService.prototype._clientSupportsPayProRefund = function() {
  ...
  if (version.agent != 'bccwc') return true;
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  try {
    Bcccore.HDPublicKey(opts.xPubKey);
  } catch (ex) {
 ...
 * @param {string} opts.unit - Bitcoin Cash unit used to format amounts in notifications.
 */
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['bcc', 'bit'], value.toLowerCase());
    },
  }];
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  var networkName = Bcccore.Address(addresses[0]).toObject().network;
 ...
 * @param {Object} opts
 * @param {string} [opts.network = Constants.LIVENET] - The Bitcoin Cash network to estimate fee levels from.
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var bcccoreTx = txp.getBcccoreTx();
    bcccoreError = bcccoreTx.getSerializationError(serializationOpts);
    if (!bcccoreError) {
      txp.fee = bcccoreTx.getFee();
    }
  } catch (ex) {
    log.error('Error building Bcccore transaction', ex);
    return ex;
  }

  if (bcccoreError instanceof Bcccore.errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (bcccoreError instanceof Bcccore.errors.Transaction.DustOutputs)
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      log.debug('Total value in all utxos (' + Utils.formatAmountInBcc(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInBcc(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      log.debug('Value after fees in all utxos (' + Utils.formatAmountInBcc(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInBcc(txpAmount) + ')');
    ...
    log.debug('Big input threshold ' + Utils.formatAmountInBcc(bigInputThreshold));
    ...
    _.each(smallInputs, function(input, i) {
      ...
      log.debug('The input contributes ' + Utils.formatAmountInBcc(netInputAmount));
      ...
      log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmountInBcc(fee));
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          ...
          log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmountInBcc((selected.length - 1) * feePerInput));
      ...
      log.debug('Cumuled total so far: ' + Utils.formatAmountInBcc(total) + ', Net total so far: ' + Utils.formatAmountInBcc(netTotal));

      if (netTotal >= txpAmount) {
        var changeAmount = Math.round(total - txpAmount - fee);
        log.debug('Tx change: ', Utils.formatAmountInBcc(changeAmount));

        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Bcccore.Transaction.DUST_AMOUNT);
        if (changeAmount > 0 && changeAmount <= dustThreshold) {
          log.debug('Change below dust threshold (' + Utils.formatAmountInBcc(dustThreshold) + '). Incrementing fee to remove change.');
    ...
    if (netTotal < txpAmount) {
      log.debug('Could not reach Txp total (' + Utils.formatAmountInBcc(txpAmount) + '), still missing: ' + Utils.formatAmountInBcc(txpAmount - netTotal));
  ...
  log.debug('Selecting inputs for a ' + Utils.formatAmountInBcc(txp.getTotalAmount()) + ' txp');
  ...
  self._getUtxosForCurrentWallet(null, function(err, utxos) {
    ...
    async.whilst(function() {
      ...
      select(candidateUtxos, function(err, selectedInputs, selectedFee) {
        ...
        log.debug('Fee for this selection: ' + Utils.formatAmountInBcc(fee));
      ...
      if (!err) {
        ...
        log.debug('Successfully built transaction. Total fees: ' + Utils.formatAmountInBcc(txp.fee) + ', total change: ' + Utils.formatAmountInBcc(change));
      } else {
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    var toAddress = {};
    try {
      toAddress = new Bcccore.Address(output.toAddress);
    } catch (ex) {
 ...
 * @param {Object} opts
 * @param {string} [opts.network = Constants.LIVENET] - The Bitcoin Cash network for this transaction.
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.copayerId, opts.signatures, copayer.xPubKey)) {
          ...
          var raw = txp.getBcccoreTx().uncheckedSerialize();
          log.warn('Raw tx:', raw);
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function getNormalizedTxs(addresses, from, to, cb) {
    ...
    var network = Bcccore.Address(addresses[0].address).toObject().network;
