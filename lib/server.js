...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new Bch.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype._verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new Bch.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  var networkName = Bch.Address(addresses[0]).toObject().network;
...
WalletService.prototype._checkTx = function(txp) {
  ...
  if (bchError instanceof Bch.errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (bchError instanceof Bch.errors.Transaction.DustOutputs)
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      log.debug('Total value in all utxos (' + Utils.formatAmountInBch(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInBch(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      log.debug('Value after fees in all utxos (' + Utils.formatAmountInBch(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInBch(txpAmount) + ')');
    ...
    log.debug('Big input threshold ' + Utils.formatAmountInBch(bigInputThreshold));
    ...
    _.each(smallInputs, function(input, i) {
      ...
      log.debug('The input contributes ' + Utils.formatAmountInBch(netInputAmount));
      ...
      log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmountInBch(fee));
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          ...
          log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmountInBch((selected.length - 1) * feePerInput));
      ...
      log.debug('Cumuled total so far: ' + Utils.formatAmountInBch(total) + ', Net total so far: ' + Utils.formatAmountInBch(netTotal));

      if (netTotal >= txpAmount) {
        var changeAmount = Math.round(total - txpAmount - fee);
        log.debug('Tx change: ', Utils.formatAmountInBch(changeAmount));

        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Bch.Transaction.DUST_AMOUNT);
        if (changeAmount > 0 && changeAmount <= dustThreshold) {
          log.debug('Change below dust threshold (' + Utils.formatAmountInBch(dustThreshold) + '). Incrementing fee to remove change.');
    ...
    if (netTotal < txpAmount) {
      log.debug('Could not reach Txp total (' + Utils.formatAmountInBch(txpAmount) + '), still missing: ' + Utils.formatAmountInBch(txpAmount - netTotal));
  ...
  log.debug('Selecting inputs for a ' + Utils.formatAmountInBch(txp.getTotalAmount()) + ' txp');
  ...
  self._getUtxosForCurrentWallet(null, function(err, utxos) {
    ...
    async.whilst(function() {
      ...
      select(candidateUtxos, function(err, selectedInputs, selectedFee) {
        ...
        log.debug('Fee for this selection: ' + Utils.formatAmountInBch(fee));
      ...
      if (!err) {
        ...
        log.debug('Successfully built transaction. Total fees: ' + Utils.formatAmountInBch(txp.fee) + ', total change: ' + Utils.formatAmountInBch(change));
      } else {
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    var toAddress = {};
    try {
      toAddress = new Bch.Address(output.toAddress);
    } catch (ex) {
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function getNormalizedTxs(addresses, from, to, cb) {
    ...
    var network = Bch.Address(addresses[0].address).toObject().network;
