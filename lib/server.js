...
/**
 * Creates an instance of the Bch Wallet Service.
 * @constructor
 */
...
/**
 * Gets the current version of BCHWS
 */
WalletService.getServiceVersion = function() {
  if (!serviceVersion)
    serviceVersion = 'bchws-' + require('../package').version;
...
WalletService.getInstance = function(opts) {
  ...
  if (version && version.agent == 'bchwc') {
    if (version.major == 0 || (version.major == 1 && version.minor < 2)) {
      throw new ClientError(Errors.codes.UPGRADE_NEEDED, 'BCHWC clients < 1.2 are no longer supported.');
    }
  }
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new bchLib.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype._verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new bchLib.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
WalletService.prototype._clientSupportsPayProRefund = function() {
  ...
  if (version.agent != 'bchwc') return true;
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  try {
    bchLib.HDPublicKey(opts.xPubKey);
  } catch (ex) {
...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['bch', 'bit'], value.toLowerCase());
    },
  }];
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  var networkName = bchLib.Address(addresses[0]).toObject().network;
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var bchTx = txp.getBchTx();
    bchError = bchTx.getSerializationError(serializationOpts);
    if (!bchError) {
      txp.fee = bchTx.getFee();
    }
  } catch (ex) {
    log.error('Error building Bch transaction', ex);
    return ex;
  }

  if (bchError instanceof bchLib.errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (bchError instanceof bchLib.errors.Transaction.DustOutputs)
  ...
  function select(utxos, cb) {
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (netTotal >= txpAmount) {
        ...
        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, bchLib.Transaction.DUST_AMOUNT);
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    var toAddress = {};
    try {
      toAddress = new bchLib.Address(output.toAddress);
    } catch (ex) {
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.copayerId, opts.signatures, copayer.xPubKey)) {
          ...
          var raw = txp.getBchTx().uncheckedSerialize();
          log.warn('Raw tx:', raw);
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function getNormalizedTxs(addresses, from, to, cb) {
    ...
    var network = bchLib.Address(addresses[0].address).toObject().network;
