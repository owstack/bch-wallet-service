...
TxProposal.create = function(opts) {
  ...
  try {
    x.network = opts.network || Bcccore.Address(x.outputs[0].toAddress).toObject().network;
  } catch (ex) {}
...
TxProposal.prototype._buildTx = function() {
  ...
  _.each(self.outputs, function(o) {
    ...
    if (o.script) {
      t.addOutput(new Bcccore.Transaction.Output({
        script: o.script,
        satoshis: o.amount
      }));
    } else {
  ...
  // Validate actual inputs vs outputs independently of Bcccore
...
TxProposal.prototype.getBcccoreTx = function() {
  var self = this;
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToBcccoreTx(t, x.signatures, x.xpub);
  });
...
TxProposal.prototype._addSignaturesToBcccoreTx = function(tx, signatures, xpub) {
  var self = this;
  ...
  var i = 0,
    x = new Bcccore.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = self.inputs[i];
    try {
      var signature = Bcccore.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(self.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: Bcccore.crypto.Signature.SIGHASH_ALL | Bcccore.crypto.Signature.SIGHASH_FORKID,
        publicKey: pub,
      };
...
TxProposal.prototype.sign = function(copayerId, signatures, xpub) {
  try {
    ...
    var tx = this.getBcccoreTx();
    this._addSignaturesToBcccoreTx(tx, signatures, xpub);
