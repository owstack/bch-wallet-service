...
TxProposal.create = function(opts) {
  ...
  try {
    x.network = opts.network || Bch.Address(x.outputs[0].toAddress).toObject().network;
  } catch (ex) {}
...
TxProposal.prototype._buildTx = function() {
  ...
  _.each(self.outputs, function(o) {
    ...
    if (o.script) {
      t.addOutput(new Bch.Transaction.Output({
        script: o.script,
        satoshis: o.amount
      }));
    } else {
  ...
  // Validate actual inputs vs outputs independently of Bch
...
TxProposal.prototype._addSignaturesToBchTx = function(tx, signatures, xpub) {
  ...
  var i = 0,
    x = new Bch.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = self.inputs[i];
    try {
      var signature = Bch.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(self.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: Bch.crypto.Signature.SIGHASH_ALL | Bch.crypto.Signature.SIGHASH_FORKID,
        publicKey: pub,
      };
