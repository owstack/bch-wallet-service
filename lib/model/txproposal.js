...
TxProposal.create = function(opts) {
  ...
  try {
    x.network = opts.network || bchLib.Address(x.outputs[0].toAddress).toObject().network;
  } catch (ex) {
    log.debug(ex);
  }
...
TxProposal.prototype._buildTx = function() {
  ...
  _.each(self.outputs, function(o) {
    ...
    if (o.script) {
      t.addOutput(new bchLib.Transaction.Output({
        script: o.script,
        satoshis: o.amount
      }));
    } else {
  ...
  // Validate actual inputs vs outputs independently of bch-lib
...
TxProposal.prototype.getBchTx = function() {
  var self = this;
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToBchTx(t, x.signatures, x.xpub);
  });
...
TxProposal.prototype._addSignaturesToBchTx = function(tx, signatures, xpub) {
  var self = this;
  ...
  var i = 0,
    x = new bchLib.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = self.inputs[i];
    try {
      var signature = bchLib.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(self.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: bchLib.crypto.Signature.SIGHASH_ALL | bchLib.crypto.Signature.SIGHASH_FORKID,
        publicKey: pub,
      };
...
TxProposal.prototype.sign = function(copayerId, signatures, xpub) {
  try {
    ...
    var tx = this.getBchTx();
    this._addSignaturesToBchTx(tx, signatures, xpub);
