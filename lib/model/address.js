...
Address.create = function(opts) {
  ...
  x.network = bchLib.Address(x.address).toObject().network;
...
Address._deriveAddress = function(scriptType, publicKeyRing, path, m, network) {
  ...
  var publicKeys = _.map(publicKeyRing, function(item) {
    var xpub = new bchLib.HDPublicKey(item.xPubKey);
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      bchAddress = bchLib.Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      bchAddress = bchLib.Address.fromPublicKey(publicKeys[0], network);
      break;
  }

  return {
    address: bchAddress.toString(),
    path: path,
