...
Address.create = function(opts) {
  ...
  x.network = Bcccore.Address(x.address).toObject().network;
...
Address._deriveAddress = function(scriptType, publicKeyRing, path, m, network) {
  ...
  var publicKeys = _.map(publicKeyRing, function(item) {
    var xpub = new Bcccore.HDPublicKey(item.xPubKey);
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      bcccoreAddress = Bcccore.Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      bcccoreAddress = Bcccore.Address.fromPublicKey(publicKeys[0], network);
      break;
  }

  return {
    address: bcccoreAddress.toString(),
    path: path,
