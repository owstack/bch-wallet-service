...
var crypto = bch.crypto;
var encoding = bch.encoding;
...
Utils.hashMessage = function(text, noReverse) {
  ...
  if (!noReverse) {
    ret = new bch.encoding.BufferReader(ret).readReverse();
  }
  return ret;
};
...
Utils.formatAmountInBcc = function(amount) {
  return Utils.formatAmount(amount, 'bch', {
    minDecimals: 8,
    maxDecimals: 8,
  }) + 'bch';
};
...
Utils.parseVersion = function(version) {
  ...
  v.agent = _.includes(['bchwc', 'bchws'], x[0]) ? 'bchwc' : x[0];
  x = x[1].split('.');
