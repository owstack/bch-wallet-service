...
var crypto = bcccore.crypto;
var encoding = bcccore.encoding;
...
Utils.hashMessage = function(text, noReverse) {
  ...
  if (!noReverse) {
    ret = new bcccore.encoding.BufferReader(ret).readReverse();
  }
  return ret;
};
...
Utils.formatAmountInBcc = function(amount) {
  return Utils.formatAmount(amount, 'bcc', {
    minDecimals: 8,
    maxDecimals: 8,
  }) + 'bcc';
};
...
Utils.formatUtxos = function(utxos) {
  ...
  return _.map([].concat(utxos), function(i) {
    var amount = Utils.formatAmountInBcc(i.satoshis);
...
Utils.parseVersion = function(version) {
  ...
  v.agent = _.contains(['bccwc', 'bccws'], x[0]) ? 'bccwc' : x[0];
  x = x[1].split('.');
