...
helpers.signMessage = function(text, privKey) {
  var priv = new Bch.PrivateKey(privKey);
  var hash = Utils.hashMessage(text);
  return Bch.crypto.ECDSA.sign(hash, priv, 'little').toString();
};

helpers.signRequestPubKey = function(requestPubKey, xPrivKey) {
  var priv = new Bch.HDPrivateKey(xPrivKey).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).privateKey;
...
helpers._generateCopayersTestData = function(n) {
  ...
  _.each(_.range(n), function(c) {
    var xpriv = new Bch.HDPrivateKey();
    var xpub = Bch.HDPublicKey(xpriv);

    var xpriv_45H = xpriv.deriveChild(45, true);
    var xpub_45H = Bch.HDPublicKey(xpriv_45H);
...
helpers.randomTXID = function() {
  return Bch.crypto.Hash.sha256(new Buffer(Math.random() * 100000)).toString('hex');;
};
      ...
      var utxos = _.compact(_.map([].concat(amounts), function(amount, i) {
        ...
        switch (wallet.addressType) {
          case Constants.SCRIPT_TYPES.P2SH:
            scriptPubKey = Bch.Script.buildMultisigOut(address.publicKeys, wallet.m).toScriptHashOut();
            break;
          case Constants.SCRIPT_TYPES.P2PKH:
            scriptPubKey = Bch.Script.buildPublicKeyHashOut(address.address);
            break;
        }
...
helpers.clientSign = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new Bch.HDPrivateKey(derivedXPrivKey, txp.network);
